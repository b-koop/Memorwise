...
const MAX_FILES_PER_SOURCE_UPLOAD = 20;
...
export const useNotebookStore = create<NotebookState>((set, get) => ({
	...
	deleteNotebook: async (id: string) => {
		...
		set((s) => ({
			...
			selectedNotebookId:
				s.selectedNotebookId === id ? null : s.selectedNotebookId,
	...
	selectNotebook: async (id: string | null) => {
		...
		if (id) {
			try {
				...
				/* network error — keep empty arrays */
			}
		}
	},
	...
	addSource: async (files: FileList | File[]) => {
		...
		try {
			...
			for (
				...
				i += MAX_FILES_PER_SOURCE_UPLOAD
			) {
				const formData = new FormData();
				formData.append("notebookId", selectedNotebookId);
				for (const file of uploadFiles.slice(
					i,
					i + MAX_FILES_PER_SOURCE_UPLOAD,
				)) {
				...
				if (!res.ok) throw new Error("Failed to add sources");
				allNewSources.push(...(await res.json()));
			}

			if (allNewSources.length > 0) {
				set((s) => ({ sources: [...allNewSources, ...s.sources] }));
			}
		} catch (err) {
