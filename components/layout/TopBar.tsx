...
export function TopBar() {
	...
	const handleSelectNotebook = async (id: string) => {
		...
		if (!nav) router.push(`/?notebook=${encodeURIComponent(id)}`);
	...
	const handleRename = async () => {
		...
		if (name && selectedNotebookId) {
			try {
				const res = await fetch(`/api/notebooks/${selectedNotebookId}`, {
					method: "PATCH",
	...
	const handleExport = async () => {
		...
		try {
			const res = await fetch(
				`/api/export?notebookId=${selectedNotebookId}&type=notebook`,
			);
			if (!res.ok) return;
			...
			a.href = url;
			a.download = `${selectedNotebook?.name || "notebook"}_export.json`;
	...
	const notebooksHref =
		!isBrain && nav
			...
			: !isBrain && selectedNotebookId
				? buildNotebookUrl("/", { notebook: selectedNotebookId })
				: "/";
		...
		<div className="h-11 min-h-11 flex items-center justify-between px-4 bg-card border-b border-border">
			...
			<div className="flex items-center gap-6">
				<img
					src="/logo-full.png"
					alt="The Stacks"
					className="h-[18px] logo-adaptive cursor-pointer"
					onClick={() => {
						selectNotebook(null);
						if (nav)
							nav.replaceNotebookUrl({
								notebook: null,
								view: "chat",
								note: null,
								session: null,
				...
				<nav className="flex items-center gap-1">
					{(
						[
							{ href: notebooksHref, label: "Notebooks", active: !isBrain },
							{ href: brainHref, label: "Brain", active: isBrain },
						] as const
					).map((section) => (
						<Link
							key={section.href}
			...
			<div className="flex items-center gap-1.5">
				...
				{!isBrain && selectedNotebookId && (
					<button
						onClick={() =>
							window.dispatchEvent(new CustomEvent("stacks:open-search"))
						}
					...
					>
						<Search size={12} />
						<span>Search</span>
						<kbd className="ml-1 px-1 py-0 text-[10px] font-medium text-foreground-muted/70 bg-card border border-border rounded">
							⌘K
						</kbd>
					</button>
				)}
							...
							{dropdownOpen && (
								...
								>
									<div className="p-1.5 max-h-64 overflow-y-auto">
											...
											<div
												...
												className={`flex items-center justify-between px-2.5 py-2 rounded-lg cursor-pointer text-[13px] transition-colors ${
													selectedNotebookId === nb.id
														? "bg-elevated text-foreground"
														: "text-foreground-secondary hover:bg-elevated/50 hover:text-foreground"
												}`}
											>
										...
										{notebooks.length === 0 && (
											<div className="px-3 py-3 text-[12px] text-foreground-muted text-center">
												No notebooks yet
											</div>
										)}
									</div>
